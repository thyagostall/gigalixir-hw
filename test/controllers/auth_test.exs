defmodule GigalixirHelloworld.AuthTest do
  ...
  setup %{conn: conn} do
    conn = conn
    |> bypass_through(GigalixirHelloworld.Router, :browser)
    |> get("/")
  ...
  test "authenticate_user continues when teh current_user exists", %{conn: conn} do
    conn = conn
  ...
  test "login with password mismatch", %{conn: conn} do
    ...
    assert {:error, :unauthorized, _conn} = Auth.login(conn, "me", "wrong_pass", repo: Repo)
  end
end
